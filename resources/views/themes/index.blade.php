					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view" >
							<div class="panel-heading">
								<div class="pull-left">
									<a href="/themes/create" class="btn btn-success"><h6 class="panel-title txt-dark">Novo Bem</h6></a>
								</div>
								<div class="pull-right">
									<form action="" class="form">
											<div class="input-group">
												...
												<span class="input-group-addon">
													<input type="radio" checked name="key" value="description" id="description" aria-label="..."> <label for="description"> Descrição</label>
												</span>
										...
										<div class="table-responsive">
											<table id="datable_1" class="table table-hover display  pb-30" >
												...
												<tbody id='tableResults'>
													<?php foreach($themes as $consorcio){

				
														?>
														<tr>
																<td>{{$consorcio->codigo}}</td>
																<td>{{$consorcio->description}}</td>
																<td>{{$consorcio->category->name}}</td>
																<td>{{$consorcio->provider->name}}</td>
																<td>{{number_format($consorcio->credito,2,',','.')}}</td>
																
																<td>
																	<a href="{{ route('themes.edit',['id'=>$consorcio->id]) }}" class="btn btn-primary btn-small">
																		Editar

																	</a>
																	<a href="{{ route('themes.destroy',['id'=>$consorcio->id]) }}" class="btn btn-warning btn-small hidden">
																			Deletar
...
<script type="text/javascript">
	function busca(){
		...
		var request = $.ajax({
		            method:"GET",
		            url:"/themes/buscarIndex/"+key+"/"+buscar,
		            dataType:"json"
		        }); 
		        ...
		        request.done(function(e){
		            ...
		            if (e.status) {
		            	...
		            	jQuery.each( e.rtn, function( i, val ) {
		            		console.log(val);
		            		msg = '<tr><td>'+val.codigo+'</td> <td>'+val.description+'</td> <td>'+val.category.name+'</td> <td>'+val.provider.name+'</td> <td>'+val.credito+'</td> <td><a href="/themes/'+val.id+'/edit" class="btn btn-primary btn-small">';
							msg+='											Editar';

							msg+='										</a> <a href="/themes/destroy?id='+val.id+'" class="btn btn-warning btn-small">';
							msg+='												Deletar';
